...
public class CaptureImagesForFirstSetOfURL {
	...
	@Test
	public void captureImageForFirstSet() throws IOException, InterruptedException {
		List<String> myStringExcelArray;
		myStringExcelArray = ReadExcel.readXLSFile(0);
		int len = myStringExcelArray.size();
		int lentemp = len;
		int i = 0;

		while (len > 0) {
			NEWRESULT_FILENAME = currentDir + "\\src\\test\\resources\\screenshots\\FirstSetOfURL" + "/"  + "screenshot" + Z + ".png";
			...
			ReadExcel.WriteXLSFile(NEWRESULT_FILENAME, rtw, 1);
